    ...
    setTimeout(() => {
      reject(
        new HeadlessClientError({
          ...
          message: `Unstable connection. Please check your internet and try again (url="${url}" timeout="${timeout}ms").`,
        }),
      )
    }, timeout)
  })
      ...
      setTimeout(() => {
        reject(
          new HeadlessClientError({
            ...
            message: `Unstable connection. Please check your internet and try again (timeout="${timeout}ms").`,
          }),
        )
      }, timeout)
